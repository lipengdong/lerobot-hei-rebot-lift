...
class LeKiwiClient(Robot):
    ...
    def _from_keyboard_to_base_action(self, pressed_keys: np.ndarray):
        ...
        if self.teleop_keys["rotate_right"] in pressed_keys:
            theta_cmd -= theta_speed
        if self.teleop_keys["height_up"] in pressed_keys:
            height_cmd += 88.0
        if self.teleop_keys["height_down"] in pressed_keys:
            height_cmd -= 88.0
        if self.teleop_keys["yao_up"] in pressed_keys:
            yao_cmd += 88.0
        if self.teleop_keys["yao_down"] in pressed_keys:
            yao_cmd -= 88.0
        return {
            "x.vel": x_cmd,
            "y.vel": y_cmd,
            "theta.vel": theta_cmd,
            "height.vel": height_cmd,
            "yao.vel": yao_cmd,
        }

...
def build_rollout_context(
    ...
    # Position joints and velocity commands are routed to the policy as state/action.
    # Mobile manipulators such as HEI ReBot Lift record base velocity commands
    # (x.vel/y.vel/theta.vel), so dropping ``.vel`` would change the learned
    # state/action dimensionality at rollout time.
